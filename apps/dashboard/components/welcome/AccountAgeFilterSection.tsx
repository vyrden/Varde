...
export function AccountAgeFilterSection({ value, onChange, roles }: AccountAgeFilterSectionProps) {
  return (
    <fieldset className="space-y-4 rounded-lg border border-border bg-muted/30 p-4">
      <legend className="px-2 text-sm font-semibold">Filtre comptes neufs</legend>
      ...
      <label className="flex cursor-pointer items-center gap-2 text-sm">
        ...
        />
        Filtrer les comptes Discord créés récemment (anti-raid basique)
      </label>
        ...
        <>
          ...
          {value.action === 'quarantine' ? (
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Crée un rôle « Quarantaine » sans permissions et configure-le ici. L'auto-rôle
                normal n'est pas appliqué quand un membre est quarantiné.
              </p>
            </div>
          ) : null}
        </>
