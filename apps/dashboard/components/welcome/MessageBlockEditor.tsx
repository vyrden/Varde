...
export interface MessageBlockEditorProps<B extends Block> {
  /** Suffixe d'IDs/labels pour éviter les collisions quand plusieurs
   * éditeurs cohabitent (welcome + goodbye sur la même page). */
 ...
 *
 * Ne rend que le formulaire — l'activation (Card + Toggle) est pilotée
 * par la section parente. On suppose que ce composant n'est rendu que
 * lorsque `block.enabled === true`.
 */
export function MessageBlockEditor<B extends Block>({
  ...
  const missingChannel = channelRequired && block.channelId === null;
    ...
    <div className="space-y-4">
      {missingChannel ? (
        <p className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-100">
          Section activée mais aucun salon sélectionné. Choisis un salon ci-dessous, sinon désactive
          la section — sans ça la sauvegarde sera refusée.
        </p>
      ) : null}
      ...
      {!isWelcome || welcomeBlock.destination !== 'dm' ? (
        <div className="space-y-1">
          <label className="block text-sm font-medium" htmlFor={`channel-${idScope}`}>
            Salon
          </label>
          <Select
            id={`channel-${idScope}`}
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <label className="block text-sm font-medium" htmlFor={`msg-${idScope}`}>
            Message
          </label>
        ...
        <textarea
          ...
          value={block.message}
          placeholder={
            isWelcome ? 'Bienvenue {user.mention} sur {guild} !' : '{user.tag} a quitté le serveur.'
          }
          maxLength={2000}
          rows={3}
            ...
            <div className="flex flex-wrap gap-1">
              {colorPresets.map((c) => (
                <button
                  key={c}
                  type="button"
                  onClick={() => updateBlock('embed', { ...block.embed, color: c } as B[keyof B])}
                  title={c}
              ...
              <div className="flex flex-wrap gap-1">
                {cardPresets.map((c) => (
                  <button
                    ...
                    title={c}
                    className={`h-6 w-6 rounded border-2 ${
                      block.card.backgroundColor === c ? 'border-foreground' : 'border-transparent'
                    }`}
                    style={{ backgroundColor: c }}
                  />
                ))}
